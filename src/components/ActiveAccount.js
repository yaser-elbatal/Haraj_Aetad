...
function ActiveAccount({ navigation, route }) {
    ...
    function onSubmit() {
        ...
        if (!err) {
            setLoader(true);
            dispatch(activate(token, code, lang, navigation)).then(() => setLoader(false)).catch(() => setLoader(false));
        }
...

                    {
                        fading ?
                            <View style={[styles.rowGroup]}>
                                <Text style={[styles.FairuzBlack, styles.textSize_15, styles.text_White, styles.marginHorizontal_10]}>
                                    {i18n.t('mon')}
                                </Text>
                                <CountDown
                                    until={60 * 1}
                                    ...
                                    digitTxtStyle={{ color: '#2E234D' }}
                                    timeLabelStyle={{ color: 'red', fontWeight: 'bold' }}
                                    separatorStyle={{ color: '#2E234D' }}
                                    timeToShow={['M', 'S']}
