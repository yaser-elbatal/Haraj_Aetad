...
import {
    ...
    Animated,
    RefreshControl,
    Keyboard, BackHandler, FlatList, ActivityIndicator, Platform
} from "react-native";
...
function Home({ navigation }) {
    ...
    const allCity = useSelector(state => state.cities.allCities);
    ...
    const [categoryUpId, setCategoryUpId] = useState(null);
    const [page, setPage] = useState(1);
    const [keyword, setKeyword] = useState('');
    const dispatch = useDispatch();

    const [upArray, setUpArray] = useState([]);
    const [upBlogs, setUpBlogs] = useState(blog);
    ...
    function selectId(type, id, name) {
        ...
        if (type === 'country') {
            ...
            const countryId = id;
            dispatch(getBlogs(lang, cityId, countryId, categoryId, latitude, longitude, 1, keyword));
        }
